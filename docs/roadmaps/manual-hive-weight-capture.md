...
So a beekeeper who does a **Kippprobe** (tilt / edge-lift with a hook scale) or a
full weighing has nowhere to record it, view its trend, or keep it on the
...
`Measurement` model** (`metric = "weight"`). Because an edge-lift is a *partial*
reading, we must capture **where** it was taken along two independent axes:
