...
1. Verify the hive sits in the context apiary (existing pattern in `HiveService.update()`).
2. **Verify the destination belongs to the user** — owner or `ACTIVE` member, the test `ApiaryContextGuard` applies to the context apiary. `ForbiddenException`, not a silent write.
